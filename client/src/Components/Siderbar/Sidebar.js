...
import ChallengesForm from '../Challenges/challengeForm';
...
export default function MiniDrawer(props) {
  ...
  const [openChallengesForm, setOpenChallengesForm] = useState(false);
  ...
  const bookNewMatch = () => setOpenBookMatch(true);

  const createTournament = () => setOpenModal(true);

  const createChallenge = () => setOpenChallengesForm(true);

  const closeModal = () => {
    setOpenModal(false);
    setOpenBookMatch(false);
    setOpenChallengesForm(false);
  }
  ...
  return (
    <div className={classes.root}>
      ...
      >
        ...
        <Divider />
        <List>
          <Tooltip title="New Match" placement="right" arrow>
            <ListItem button key="k1" onClick={() => bookNewMatch()}>
              <ListItemIcon><SportsSoccerIcon color="error" /></ListItemIcon>
              <ListItemText primary="Book New Match" />
            </ListItem>
          </Tooltip>
          <Tooltip title="Create Tournament" placement="right" arrow>
            <ListItem button key="k2" onClick={() => createTournament()}>
              <ListItemIcon><EmojiEventsIcon fontSize="medium" style={{color: '#F4D03F'}} /></ListItemIcon>
              <ListItemText primary="Create Tournament" />
            </ListItem>
          </Tooltip>
          <Tooltip title="Create Challenge" placement="right" arrow>
            <ListItem button key="k3" onClick={ () => createChallenge()}>
              <ListItemIcon><img src="https://img.icons8.com/dusk/34/000000/football.png"/></ListItemIcon>
              <ListItemText primary="Create Challenge" />
            </ListItem>
          </Tooltip>
          <Tooltip title="My Team" placement="right" arrow>
            <ListItem button key="k4" onClick={() => history.push('/myTeam')}>
              <ListItemIcon><GroupWorkIcon fontSize="medium" style={{color: '#F4D03F'}} /></ListItemIcon>
        ...
        <Divider />
        <List>
          <Tooltip title="My Challenges" placement="right" arrow>
          ...
          <Tooltip title="Logout" placement="right" arrow>
            <ListItem button key="k6" onClick={logout}>
      ...
      <main className={classes.content} data-aos="zoom-in">
        ...
        <ChallengesForm open={openChallengesForm} Close={closeModal} />
        ...
        <div className="train">
          <div className="row w-100">
            <div className="col-lg-6 text-white">
              <h2 className="mb-4">Challenge Other Teams</h2>
              <p className="lead">
                In Mal3abi Application, you can Challenge your friends, play with other teams.
              </p>
            </div>
